...
void _jdz_span_push_deferred_free_list(span_t *span, void *block) {
    ...
    while (1) {
        *((void**)block) = &span->deferred_free_list;
        ...
        if (atomic_compare_exchange_weak_explicit(&span->deferred_free_list, (void**)block, block, memory_order_relaxed, memory_order_relaxed)) {
            atomic_fetch_add_explicit(&span->deferred_frees, 1, memory_order_relaxed);
...
void _jdz_span_initialise_fresh_span(span_t *span, arena_t *arena, size_class_t size_class) {
    ...
    span->alloc_ptr = ((uintptr_t) span) + SPAN_HEADER_SIZE;
...
span_t* _jdz_span_split_first_spans_return_remaining(span_t *span, size_t span_count) {
    ...
    span_t *remaining_span = (span_t*) ((uintptr_t) span + SPAN_SIZE * span_count);

    remaining_span->span_count = span->span_count - span_count;
    remaining_span->alloc_size = span->alloc_size - ((uintptr_t) remaining_span - span->initial_ptr);
    remaining_span->initial_ptr = (uintptr_t) remaining_span;
...
static void* _jdz_span_alloc_from_alloc_ptr(span_t *span) {
    assert(span->alloc_ptr <= (uintptr_t) span + SPAN_SIZE - span->class.block_size);
